		...
		<div class="container">
			<div class="header">
				...
				<div class="top-nav">
					...
					<ul class="nav1">
						<li class="hvr-sweep-to-bottom"><a href="index_customer.jsp">Home<i><img
									src="images/nav-but1.png" alt="" /></i></a></li>					
...
<%
		ArrayList<orderdetail> list = (ArrayList<orderdetail>)request.getAttribute("listProduct");
		String message = (String)request.getAttribute("notfound")==null? "":(String)request.getAttribute("notfound");
%>
<%=message %> 
<%if(message != null){ %>
<script type="text/javascript">
...
<form action="selectProductCustomer" method="post">
									<ul>
										<li><input type="number" placeholder="กรอก id order " name="id" min="0">
										</li>
										<li><input type="hidden" name="customerId" value="<%=list.get(0).getCustomer().getCustomerId()%>"></li>
								...
								<div class="CSSTableGenerator"
									...
									<table>
										...
										<tr>
											...
											<%if(order.getOrder().getStatus().equals("complete")){ %>		
											<%if(order.getStatusCustomer().equals("0")){ %>
											<td><a href="confirmStatusCustomer?idcus=<%=order.getCustomer().getCustomerId()%>&idpro=<%=order.getProduct().getProductId()%>&idorder=<%=order.getOrder().getOrderId()%>"><input type="button" value="ยืนยันการรับสินค้า"></a></td>
											<%}else{ %>
												<td>รับสินค้าแล้ว</td>
											<%} %>
											<%}else{ %>
											<td>สินค้ายังไม่ถึงจังหวัดของท่าน</td>
											<%} %>
										</tr>
										<%
										i++;
						...
						<h3 class="wow fadeInRight animated" data-wow-delay="0.4s"
							style="visibility: visible; -webkit-animation-delay: 0.4s;">ยอดชำระรวม
							 : <%=total %></h3>
